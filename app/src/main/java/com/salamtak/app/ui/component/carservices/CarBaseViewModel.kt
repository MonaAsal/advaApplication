package com.salamtak.app.ui.component.carservices
 ...
 */
@HiltViewModel
open class CarBaseViewModel @Inject
constructor(val dataRepository: DataRepository) :
    ...
    fun getInstallmentsLookup() {
        viewModelScope.launch {
            ...
            when (val resource =
                dataRepository.getInstallmentsLookup(InstallmentTypes.CARS.typeId.toString())) {
                is Resource.Success -> {
                    showLoading.postValue(false)
                    resource.data?.let {
                        installmentTypes.value = it.carServiceInstallments!!
                        selectInstallmentType(it.carServiceInstallments[0])
                    }
                }
