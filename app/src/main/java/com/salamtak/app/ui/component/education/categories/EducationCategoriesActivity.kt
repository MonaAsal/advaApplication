...
import com.salamtak.app.ui.component.education.custom.EducationActivity
...
import kotlinx.android.synthetic.main.activity_categories.progress_bar
...
@AndroidEntryPoint
class EducationCategoriesActivity : BaseActivity(), RecyclerItemListener<Category> {
    //    @Inject
//    lateinit
...
//    lateinit var viewModelFactory: ViewModelFactory
    ...
    override val layoutId: Int
        get() = R.layout.activity_education_categories
...
//        catgoriesListViewModel = viewModelFactory.create(EducationCategoriesViewModel::class.java)
    }
...
//            tv_add_financial_profile.visibility = View.GONE
       ...
       // tv_add_financial_profile.setOnClickListener { navigateToFinantialInfoScreen() }
...
//        EspressoIdlingResource.decrement()
    ...
    private fun navigateToDetailsScreen(navigateEvent: Event<Category>) {
        navigateEvent.getContentIfNotHandled()?.let {
            ...
            //startActivity(intentFor<SubcategoryActivity>(Constants.CATEGORY_ITEM_KEY to it))
        }
    }
...
//            13 -> LogAdjustEvent("gicrvq")
    ...
    private fun showDataView(show: Boolean) {
        ...
        rv_categories_list.visibility = if (show) VISIBLE else GONE
        progress_bar.toGone()
    }
    ...
    override fun onItemSelected(item: Category) {
        when (item.id!!) {
            EducationTypes.School.value -> {
                startActivity<SchoolsActivity>()
            }
            EducationTypes.University.value -> {
                startActivity(intentFor<UniversitiesInstitutesActivity>(Constants.KEY_CATEGORY_ID to EducationTypes.University.value))
            }
            EducationTypes.Institute.value -> {
                startActivity(intentFor<UniversitiesInstitutesActivity>(Constants.KEY_CATEGORY_ID to EducationTypes.Institute.value))
            }
            ...
            EducationTypes.COURSES.value -> {
                startActivity(intentFor<SchoolsActivity>(Constants.KEY_CATEGORY_ID to EducationTypes.COURSES.value))
            }
        }
    }
